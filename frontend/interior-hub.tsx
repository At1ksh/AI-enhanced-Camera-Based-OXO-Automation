...
interface ComponentEntry {
  name: string;
  image: string; // cached as URI
}

export default function InteriorHub() {
  ...
  const loadComponentsAndStatuses = async () => {
    try {
      // ✅ Load cached dynamic components
      const cached = await AsyncStorage.getItem('interior_components');
      console.log("🔍 Raw cached interior_components:", cached);
      ...
      if (Object.keys(parsed).length > 0) {
        console.log("🔍 First component sample:", parsed[Object.keys(parsed)[0]]);
      }
      ...
      // ✅ Load counts
      const p = await AsyncStorage.getItem('interior_pending_count');
      const o = await AsyncStorage.getItem('interior_ok_count');
      const n = await AsyncStorage.getItem('interior_notok_count');
      setPendingCount(parseInt(p || '0'));
      setOkCount(parseInt(o || '0'));
      setNotOkCount(parseInt(n || '0'));
    } catch (e) {
      console.error('❌ Failed to load components or statuses:', e);
    }
  };
  ...
  const renderStatus = (sysname: string) => {
    const [okCount, notOkCount] = resultCounts[sysname] || [0, 0];
    if (okCount === 0 && notOkCount === 0) {
      return <Text style={styles.pending}>🟡 Status: Pending</Text>;
    }
    if (okCount > 0 && notOkCount === 0) {
      return <Text style={styles.ok}>🟢 OK: {okCount}</Text>;
    }
    if (okCount === 0 && notOkCount > 0) {
      return <Text style={styles.notok}>🔴 NOT OK: {notOkCount}</Text>;
    }
    return (
    ...
    >
      <BlurView intensity={80} style={styles.blurContainer}>
        <View style={styles.container}>
                ...
                <TouchableOpacity
                  ...
                  onPress={() =>
                    ...
                    router.push({
                      ...
                      params: { component: sysname, module: 'interior' },
                    })
                  }
                >
          ...
          <TouchableOpacity
            style={[styles.backButton,{marginBottom: insets.bottom + 12}]}
...
const styles = StyleSheet.create({
  ...
  pending: { color: '#e69500', fontWeight: 'bold' },
  ok: { color: '#2e8b57', fontWeight: 'bold' },
  notok: { color: '#d32f2f', fontWeight: 'bold' },
  statusLine: { fontSize: 14, fontWeight: '500', marginTop: 4 },
  ...
  status: { fontSize: 15 },
  lockedCard: { opacity: 0.6, borderColor: '#28a745', borderWidth: 2 },
  lockedText: { color: '#28a745', fontWeight: 'bold', marginTop: 6, fontSize: 13 },
  backButton: {
