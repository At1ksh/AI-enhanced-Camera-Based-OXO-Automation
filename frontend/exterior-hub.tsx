...
export default function ExteriorHub() {
  ...
  useEffect(() => {
    const loadComponentsAndStatuses = async () => {
      try {
        const cached = await AsyncStorage.getItem('exterior_components');
        console.log("🔍 Loaded exterior_components:", cached);
        ...
        const p = await AsyncStorage.getItem('exterior_pending_count');
        const o = await AsyncStorage.getItem('exterior_ok_count');
        const n = await AsyncStorage.getItem('exterior_notok_count');
        setPendingCount(parseInt(p || '0'));
        setOkCount(parseInt(o || '0'));
        setNotOkCount(parseInt(n || '0'));
      } catch (e) {
        console.error('❌ Failed to load exterior components or statuses:', e);
      }
    };
                ...
                <TouchableOpacity
                  ...
                  onPress={() =>
                    ...
                    router.push({
                      ...
                      params: { component: sysname, module: 'exterior' },
                    })
                  }
                >
